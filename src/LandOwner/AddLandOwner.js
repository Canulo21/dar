...
function AddLandOwner({ setShowModal, fetchUpdatedData }) {
  ...
  const handleRegistration = async (e) => {
    ...
    try {
      await axios.post("http://localhost:8080/addLandOwner", formData);
      ...
      // Call the function passed from LandOwnerDetails to fetch updated data
      fetchUpdatedData();
  ...
  useEffect(() => {
    // Update filterProvince when getProv changes
    if (getProv) {
    ...
    <>
      <div className="w-full">
        <motion.div
          ...
          <div>
            <h1 className="text-[#6b7280]">Add Owner here</h1>
